...
class Document extends Component
{
    ...
    #[Rule('file|mimes:pdf|max:1024000')]
    public $document;

    // Additional properties used in the component
    public $path;
    public $totalDueDocuments;
    public $volume;
    ...
    public function editReferenceNumber($documentId)
    {
        $this->resetErrorBag();
        $this->resetValidation();
        $this->editingDocumentId = $documentId;
        $this->editingReferenceNumber = ''; // Initialize a property to hold the editing reference number
        $this->showEditModal = true; // Open the modal
        $this->isEditModal = true; // Set the flag for edit mode
    }
    ...
    public function updateReferenceNumber()
    {
        ...
        if ($document) {
            ...
            // Refresh the documents list or any other necessary updates
    ...
    public function cancelEditReferenceNumber()
    {
        ...
        $this->createForm = !$this->isEditModal; // Reopen the file upload form only if it's not in edit mode
        $this->isEditModal = false; // Reset the edit mode flag
    }

    public function refreshDocuments()
    {
        // Logic to refresh the documents list, you can implement this based on your requirements
    }
    ...
    public function mount(){
        ...
        $this->updateTotalDueDocumentsCount();
    }
   ...
   // Delete a document and move it to the recycle bin
   public function deleteDocument($documentId)
{
    ...
    if ($document) {
        ...
        // Soft delete the document and create a Recycle entry
        ...
        if ($result) {
            ...
            // Flash success messages
            session()->flash('status', 'Document moved to recycle bin successfully');
            session()->flash('success', 'Document deleted successfully');
        } else {
            // Flash an error message if document deletion fails
            session()->flash('status', 'Error deleting document');
        }
    } else {
        // Flash an error message if the document is not found
        session()->flash('status', 'Document not found');
    }
    ...
    public function checkDueDates()
    {
        ...
        foreach ($documentsAlmostDue as $document) {
            ...
            $this->generateAlert("Document '{$document->document}' is almost dued.");
    ...
    protected function updateTotalDueDocumentsCount()
    {
        // Get the count of documents with due dates that have already passed and not alerted
        $this->totalDueDocuments = ModelsDocument::where('due_date', '<', now())
            ->where('total_due', 0)
            ->count();
    }
    public function generateReport()
    {
        ...
        // For example, you can create a PDF report with document details
